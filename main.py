...
def choose_num_players(screen):
    ...
    while True:
        ...
        for i, num in enumerate(options):
            ...
            pygame.draw.rect(screen, BEIGE, btn_rect)
            text = font.render(str(num), True, DARK_GREY)
            screen.blit(text, (btn_x + 35, btn_y + 15))
...
def initialize_game():
    ...
    return board, players,dice, 0, None, False


def handle_events(players, current_player, rolling_dice, board,dice):
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            return False, rolling_dice, None, current_player
        elif event.type == pygame.MOUSEBUTTONDOWN:
            rolling_dice, dice_result, current_player = process_click(players, current_player, rolling_dice, board,dice)
            return True, rolling_dice, dice_result, current_player
    return True, rolling_dice, None, current_player


def process_click(players, current_player, rolling_dice, board,dice):
    ...
    if x > INFO_PANEL_X:
        if not rolling_dice:
            rolling_dice = True
            dice.roll(window)
            dice_result = dice.value
            players[current_player].find_possible_moves(board, dice_result)
...
def draw_game(screen, board, players, current_player, dice_result, dice):
    screen.fill(BEIGE)
...
def main():
    ...
    board, players,dice, current_player, dice_result, rolling_dice = initialize_game()
    ...
    while running:
        ...
        draw_game(window, board, players, current_player, dice_result, dice)
        clock.tick(FPS)
